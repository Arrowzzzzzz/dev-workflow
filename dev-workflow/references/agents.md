...
- **初始化与同步**：首次进入项目时按总纲「调用时初始化与产物分层」就位基础产物（config.yaml 等）；config.yaml 变更后按第五节 5.6 自动同步到各工具配置。
...
【模型】该角色在本工具（opencode）应使用 <config.yaml 中 models.frontend.opencode 配置的模型>；
        未配置或本工具不支持切换时使用当前默认模型并说明
```
...
### 5.6 自动同步（config.yaml → 各工具配置）

config.yaml 是声明源；**各工具的原生配置不会自动感知它的变更**——同步由本技能在运行时以指令方式自动完成：

- **触发时机**：每次读取 config.yaml、派发角色任务前，以及 config.yaml 被修改后；
- **同步动作**：逐工具比对落点文件与 `models.<role>.<tool>` 的模型值——落点缺失或不一致时自动重生成对应角色段（生成规则与模板见 5.2 / 5.3 与 `assets/adapters/`）；
- **边界**：只新增/更新本技能管理的角色键，不改动用户的其他配置；opencode 的 `opencode.jsonc` / `opencode.json` 均支持（保留原有注释与配置项）；
- **快照类整体重生成**：`.devflow/agents/adapters/dsh-subagents.yaml` 与 `codex-agents.json` 允许整体重写（可用可选脚本 `assets/adapters/sync.py`，也可由执行 AI 直接重生成）；
- **降级**：无权限写入用户级目录（如 `~/.zcode/`）或无法定位工具配置时，跳过该工具落点并在任务声明中注明"仅 prompt 注入通道"；
- 同步结果在任务声明中注明（如"已同步 frontend@opencode 至 opencode.jsonc"）。
